# https://github.com/facebookresearch/tdfbanks/blob/master/melfilters.py
...
def gabor(nfilters, f_min=40, f_max=10000, fs=22050, wlen=25, nfft=2048):
    ...
    filters = gabor_object.gaborfilters
    filters = [f for fltr in filters for f in [np.real(fltr), np.imag(fltr)]]
    return filters
...
class Gabor(object):
    ...
    def _gabor_wavelet(self, eta, sigma):
        ...
        return np.asarray([gabor_function(t) for t in np.arange(-self.wlen//2, self.wlen//2)])
