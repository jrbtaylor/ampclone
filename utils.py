...
def prefilter(wav_file, saveto, n_filters=40, f_min=40, f_max=16000, fs=44100):
    ...
    filters = filterbank(n_filters, f_min, f_max, fs)
    for idx, (b, a) in enumerate(filters):
        x = lfilter(b, a, signal)
        save_wav(x, saveto + '_%i' % idx + '.wav', fs)
...
def split_signal(data):
    ...
    split_length = int(np.ceil(data.size / (data.size // 8e4)))  # Try this for IIR filter?
